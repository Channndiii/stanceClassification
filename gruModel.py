...
class TextGRU(object):
    def __init__(self, wordEmbedding, textLength, embedding_size, hidden_size=128, class_num=2):
        ...
        for inputs in [quote_inputs, response_inputs]:
            with tf.variable_scope('GRU-%s' % inputs.name.split('/')[-1][:-2]):
                ...
                gru_cell = tf.contrib.rnn.DropoutWrapper(cell=gru_cell, input_keep_prob=1.0, output_keep_prob=self.dropout_keep_prob)
                init_state = gru_cell.zero_state(self.batch_size, dtype=tf.float32)
                output, _ = tf.nn.dynamic_rnn(gru_cell, inputs, initial_state=init_state, time_major=False)
...
if __name__ == '__main__':
    ...
    model = TextGRU(wordEmbedding, textLength=max_len, embedding_size=300)
    ...
    model_save_path = './ckpt/gruModel/gru.ckpt'
    tr_batch_size = 128
    decay = 0.85
    # max_epoch = 5
    # max_max_epoch = 12
