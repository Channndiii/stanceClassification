...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        if self.attention_mechanism['Type'] != 'both_concat':
            ...
            self.main_out = nn.Linear(in_features=self.hidden_size * 2 * 2 * 2, out_features=self.class_num)
            self.sup_out = nn.Linear(in_features=self.hidden_size * 2 * 2 * 2, out_features=self.class_num)
    ...
    def forward(self, X_q_inputs, X_r_inputs, q_init_state, r_init_state, taskFlag):
        ...
        if taskFlag == 'main':
            output = self.main_out(self.dropout(concat_outputs))
        else:
            output = self.sup_out(self.dropout(concat_outputs))
        return output
...
def train_epoch(model, data_train, taskFlag):
    ...
    for batch in range(tr_batch_num):
        ...
        output = model(X_q_batch, X_r_batch, q_init_state, r_init_state, taskFlag)
    ...
    test_acc, test_cost, test_cr = test_epoch(model, main_test)

    print('**Test {}, acc={:.6f}, cost={:.6f}\n'.format(main_test.y.shape[0], test_acc, test_cost))
...
def train_batch(model, data_train, taskFlag):
    model.train()
    ...
    # valid_acc, valid_cost, _ = test_epoch(model, main_test)
    # print('BATCH {} training {} task acc={:.6f}, cost={:.6f}, test acc={:.6f}, cost={:.6f}'.format(batch, taskFlag, _acc, _cost.data[0], valid_acc, valid_cost))
    # fw.write('BATCH {} training {} task acc={:.6f}, cost={:.6f}, test acc={:.6f}, cost={:.6f}\n'.format(batch, taskFlag, _acc, _cost.data[0], valid_acc, valid_cost))

    valid_acc, valid_cost, disagree_f1, agree_f1 = test_epoch(model, main_test)
    average_f1 = (disagree_f1 + agree_f1) / 2
    print('BATCH {} training {} task acc={:.6f}, cost={:.6f}, test acc={:.6f}, cost={:.6f}, disagree_f1={:.6f}, agree_f1={:.6f}, average_f1={:.6f}'.format(batch, taskFlag, _acc, _cost.data[0], valid_acc, valid_cost, disagree_f1, agree_f1, average_f1))
    fw.write('BATCH {} training {} task acc={:.6f}, cost={:.6f}, test acc={:.6f}, cost={:.6f}, disagree_f1={:.6f}, agree_f1={:.6f}, average_f1={:.6f}\n'.format(batch, taskFlag, _acc, _cost.data[0], valid_acc, valid_cost, disagree_f1, agree_f1, average_f1))
...
def test_epoch(model, dataset):
    ...
    for i in range(batch_num):
        ...
        test_output = model(X_q_batch, X_r_batch, q_init_state, r_init_state, taskFlag='main')
        ...
        if i == 0:
            ...
            _pred = np.concatenate((_pred, y_pred.data.cpu().numpy())) if USE_GPU else np.concatenate(
                (_pred, y_pred.data.numpy()))
            _true = np.concatenate((_true, y_batch.data.cpu().numpy())) if USE_GPU else np.concatenate(
...
if __name__ == '__main__':
    ...
    USE_GPU = True
    data_name = 'iac'
    with open('./data/%s_main_64.pkl' % data_name, 'rb') as fr:
        X_main = pickle.load(fr)
        y_main = pickle.load(fr)
        word2id = pickle.load(fr)
        id2word = pickle.load(fr)
        label2id = pickle.load(fr)
        id2label = pickle.load(fr)
    with open('./data/%s_sup_64.pkl' % data_name, 'rb') as fr:
        X_sup = pickle.load(fr)
        y_sup = pickle.load(fr)
        word2id = pickle.load(fr)
        id2word = pickle.load(fr)
        label2id = pickle.load(fr)
        id2label = pickle.load(fr)
    ...
    np.random.seed(12)
    new_index = np.random.permutation(len(X_main))
    X_main = X_main[new_index]
    y_main = y_main[new_index]
    ...
    for type in ['cross_attention']:
        ...
        for i, (train_index, test_index) in enumerate(kf.split(X_main)):
            if i != 4:
                continue
            with open('./result_log/%s_MTL_BBB_%s_%s.txt' % (data_name, type, i), 'w') as fw:
                if USE_GPU:
                ...
                y_main_train, y_main_test = y_main[train_index], y_main[test_index]
                ...
                print('X_train.shape={}, y_train.shape={};\nX_test.shape={}, y_test.shape={}'.format(X_main_train.shape, y_main_train.shape, X_main_test.shape, y_main_test.shape))
                fw.write('X_train.shape={}, y_train.shape={};\nX_test.shape={}, y_test.shape={}\n'.format(X_main_train.shape, y_main_train.shape, X_main_test.shape, y_main_test.shape))

                print('Creating the data generator ...')
                fw.write('Creating the data generator ...\n')

                main_train = multiTask_data_helper_2018.BatchGenerator(X_main_train, y_main_train, shuffle=True)
                main_test = multiTask_data_helper_2018.BatchGenerator(X_main_test, y_main_test, shuffle=False)

                sup_train = multiTask_data_helper_2018.BatchGenerator(X_sup_train, y_sup_train, shuffle=True)
                ...
                #         fw.write('EPOCH {}, lr={}, training Main Task\n'.format(epoch, lr))
                #         train_epoch(model, main_train, taskFlag='main')
                #     else:
                #         print('EPOCH {}, lr={}, training Sup Task'.format(epoch, lr))
                #         fw.write('EPOCH {}, lr={}, training Sup Task\n'.format(epoch, lr))
                #         train_epoch(model, sup_train, taskFlag='sup')
                ...
                for batch in range(1, 50000 + 1):
                    ...
                    # if batch % 2 != 0:
                    # if batch > 0:
                    #     train_batch(model, main_train, taskFlag='main')
                    # else:
                    #     train_batch(model, sup_train, taskFlag='supp')
