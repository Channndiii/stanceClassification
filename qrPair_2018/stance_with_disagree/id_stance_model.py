...
class TextBiLSTM(nn.Module):
    ...
    def forward(self, X_inputs, init_state):

        # quote_inputs = self.dropout(self.embedding(X_q_inputs))
        # response_inputs = self.dropout(self.embedding(X_r_inputs))
        inputs = self.embedding(X_inputs)
        outputs, _ = self.bilstm(inputs, init_state)
...
class AttentionModel(nn.Module):
    ...
    def build_attention_layer(self, attention_type_detail):
        ...
        if attention_type_detail.find('dot') == -1:
            ...
            setattr(self, 't2t_attention_layer', attention_layer)
        else:
            ...
            setattr(self, 'self_dot_t2t_attention_layer', quote_attention_layer)

    def forward(self, X_inputs, init_state):
        outputs = self.bilstm.forward(X_inputs, init_state)

        if self.attention_mechanism['Type'] != 'None':
            attention_type_detail = self.attention_mechanism['TypeDetail']
            outputs = self.get_attention_result(
    ...
    def get_attention_result(self, attention_type_detail, outputs):

        if attention_type_detail == 'self':
            _outputs = self.attention_layer(
                self.t2t_attention_layer,
                bilstm_outputs=outputs,
                attention_source=outputs
            )
        else:
            _outputs = self.self_dot_attention_layer(
                layer=self.self_dot_t2t_attention_layer,
                bilstm_outputs=outputs
            )
        return _outputs
...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        self.single_out = nn.Sequential(
            ...
            self.out_actfuc,
            nn.Dropout(self.drop_prob),
            nn.Linear(in_features=self.hidden_size, out_features=self.class_num)
            )
    ...
    def forward(self, X_inputs, init_state):
        ...
        outputs = self.model.forward(X_inputs, init_state)
        output = self.single_out(outputs)
        return output
...
def train_batch(model, data_train):
    ...
    X_batch = Variable(torch.LongTensor(X_batch)).cuda() if USE_GPU else Variable(torch.LongTensor(X_batch))
    y_batch = Variable(torch.LongTensor(y_batch)).cuda() if USE_GPU else Variable(torch.LongTensor(y_batch))

    init_state = model.init_hidden(y_batch.size()[0])

    output = model(X_batch, init_state)
    cost = loss_func(output, y_batch)

    y_pred = torch.max(output, 1)[1]

    stance_acc = sum(y_pred.data.cpu() == y_batch.data.cpu()) / float(y_batch.size(0)) if USE_GPU else sum(y_pred.data == y_batch.data) / float(y_batch.size(0))
...
def test_epoch(model, dataset):
    ...
    X_batch = Variable(torch.LongTensor(X_batch), volatile=True).cuda() if USE_GPU else Variable(torch.LongTensor(X_batch), volatile=True)
    y_batch = Variable(torch.LongTensor(y_batch)).cuda() if USE_GPU else Variable(torch.LongTensor(y_batch))
    ...
    stance_true = y_batch.data.cpu().numpy()
    stance_pred = y_pred.data.cpu().numpy()
    ...
    for topic in ['evolution', 'abortion', 'gun control', 'gay marriage', 'existence of God']:
        ...
        for setting in settings:
            ...
            config = {
                ...
                'attention_mechanism': attention_mechanism_config}

            KFold_BEST = []
            for i, (train_index, test_index) in enumerate(kf.split(X)):
                with open('./result_log/%s_%s_%s_%s_%s_indpd.txt' % (data_type, topic, type, type_detail, i), 'w') as fw:
                    if USE_GPU:
            ...
            with open('./result_log/%s_%s_result_summary_indpd.txt' % (data_type, topic), 'a') as fw:
