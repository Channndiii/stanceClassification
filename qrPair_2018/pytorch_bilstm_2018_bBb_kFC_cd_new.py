...
def train_batch(model, data_train):
    ...
    valid_acc, valid_cost, disagree_f1, neural_f1, agree_f1 = test_epoch(model, data_test)
    average_f1 = (disagree_f1 + neural_f1 + agree_f1) / 3
    print('BATCH {} training acc={:.6f}, cost={:.6f}, test acc={:.6f}, cost={:.6f}, disagree_f1={:.6f}, neural_f1={:.6f}, agree_f1={:.6f}, average_f1={:.6f}'.format(batch, _acc, _cost.data[0], valid_acc, valid_cost, disagree_f1, neural_f1, agree_f1, average_f1))
    fw.write('BATCH {} training acc={:.6f}, cost={:.6f}, test acc={:.6f}, cost={:.6f}, disagree_f1={:.6f}, neural_f1={:.6f}, agree_f1={:.6f}, average_f1={:.6f}\n'.format(batch, _acc, _cost.data[0], valid_acc, valid_cost, disagree_f1, neural_f1, agree_f1, average_f1))
    return average_f1
...
def test_epoch(model, dataset):
    ...
    return mean_acc, mean_cost, f1[0], f1[1], f1[2]
...
if __name__ == '__main__':
    ...
    # settings = ['hybrid, self_dot, cross_dot_v2, 3']
    ...
    for setting in settings:
        ...
        config = {
            'word2id': word2id, 'pretrain_emb': True, 'class_num': 3,
        ...
        for i in range(cv_times):
            # if i != 1:
            #     continue
            with open('./result_log/create_debate_BBB_%s_%s_%s_0517.txt' % (type, type_detail, i), 'w') as fw:
        ...
        with open('./result_log/create_debate_result_summary_0517.txt', 'a') as fw:
            print('{} {}'.format(type, type_detail))
            fw.write('{} {}\n'.format(type, type_detail))
            for item in KFold_BEST:
                print(item)
                fw.write('{}\n'.format(item))
            print('Average_F1={:.6f}'.format(sum([f1[2] for f1 in KFold_BEST]) / len(KFold_BEST)))
            fw.write('Average_F1={:.6f}\n'.format(sum([f1[2] for f1 in KFold_BEST]) / len(KFold_BEST)))
