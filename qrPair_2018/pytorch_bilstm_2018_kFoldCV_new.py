...
class AttentionModel(nn.Module):
    def __init__(self, config):
        ...
        if self.do_BN:
            if self.attention_mechanism['Type'] == 'both_concat':
                ...
                self.quote_output_BN = nn.BatchNorm1d(num_features=self.hidden_size * 2 * 3)
                self.response_output_BN = nn.BatchNorm1d(num_features=self.hidden_size * 2 * 3)
            else:
        ...
        if self.attention_mechanism['Type'] != 'None':
            ...
            elif self.attention_mechanism['Type'] == 'match':
                self.quote2response_attention_layer = nn.Linear(in_features=64, out_features=1)
                self.response2quote_attention_layer = nn.Linear(in_features=64, out_features=1)
    ...
    def forward(self, X_q_inputs, X_r_inputs, q_init_state, r_init_state):
        ...
        if self.attention_mechanism['Type'] != 'None':
            ...
            if self.attention_mechanism['Type'] == 'both_concat':
                ...
                quote_outputs = _quote_outputs
                response_outputs = _response_outputs
            if self.attention_mechanism['Type'] == 'hybrid':
                quote_outputs_self = self.attention_layer(self.quote2quote_attention_layer, bilstm_outputs=quote_outputs, attention_source=quote_outputs, ActFunc=ActFunc)
                quote_outputs_cross = self.attention_layer(self.response2quote_attention_layer, bilstm_outputs=quote_outputs, attention_source=response_outputs, ActFunc=ActFunc)
                quote_outputs_self_add_cross = quote_outputs_self + quote_outputs_cross

                response_outputs_self = self.attention_layer(self.response2response_attention_layer, bilstm_outputs=response_outputs, attention_source=response_outputs, ActFunc=ActFunc)
                response_outputs_cross = self.attention_layer(self.quote2response_attention_layer, bilstm_outputs=response_outputs, attention_source=quote_outputs, ActFunc=ActFunc)
                response_outputs_self_add_cross = response_outputs_self + response_outputs_cross

                # quote_outputs_sum = torch.sum(quote_outputs, dim=1)
                # response_outputs_sum = torch.sum(response_outputs, dim=1)

                quote_outputs = torch.cat((quote_outputs_self, quote_outputs_cross, quote_outputs_self_add_cross), dim=1)
                response_outputs = torch.cat((response_outputs_self, response_outputs_cross, response_outputs_self_add_cross), dim=1)
            if self.attention_mechanism['Type'] == 'share':
                ...
                quote_outputs = _quote_outputs
                response_outputs = _response_outputs
            if self.attention_mechanism['Type'] == 'match':
                quote_outputs, response_outputs = self.attention_layer_new(self.quote2response_attention_layer, self.response2quote_attention_layer, quote_outputs, response_outputs, ActFunc)
        else:
    ...
    def attention_layer_new(self, q2r_layer, r2q_layer, quote_outputs, response_outputs, ActFunc):

        size = quote_outputs.size()
        response_outputs_T = response_outputs.transpose(1, 2).contiguous()
        transition_matrix = torch.bmm(quote_outputs, response_outputs_T)

        # response2quote_logits = ActFunc(r2q_layer(transition_matrix.view(-1, size[1]))).view(-1, size[1], 1).transpose(1, 2).contiguous()
        # response2quote_logits = response2quote_logits.view(-1, size[1])
        # response2quote_signals = F.softmax(response2quote_logits, dim=1).view(-1, 1, size[1])

        response2quote_logits = ActFunc(r2q_layer(transition_matrix.view(-1, size[1]))).view(-1, size[1], 1).transpose(1, 2).contiguous()
        response2quote_logits = response2quote_logits.view(-1, size[1])
        response2quote_signals = F.softmax(response2quote_logits, dim=1).view(-1, 1, size[1])

        quote2response_logits = ActFunc(q2r_layer(transition_matrix.transpose(1, 2).contiguous().view(-1, size[1]))).view(-1, size[1], 1).transpose(1, 2).contiguous()
        quote2response_logits = quote2response_logits.view(-1, size[1])
        quote2response_signals = F.softmax(quote2response_logits, dim=1).view(-1, 1, size[1])

        # response2quote_signals = F.softmax(torch.sum(transition_matrix, dim=2), dim=1).view(size[0], 1, -1)
        # quote2response_signals = F.softmax(torch.sum(transition_matrix, dim=1), dim=1).view(size[0], 1, -1)

        return torch.bmm(response2quote_signals, quote_outputs).view(-1, quote_outputs.size()[2]), torch.bmm(quote2response_signals, response_outputs).view(-1, response_outputs.size()[2])
...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        if self.attention_mechanism['Type'] == 'both_concat':
            ...
            self.out = nn.Linear(in_features=self.hidden_size * 2 * 3 * 2, out_features=self.class_num)
        else:
        ...
        if self.do_BN:
            ...
            if self.attention_mechanism['Type'] == 'both_concat':
                ...
                self.concat_output_BN = nn.BatchNorm1d(num_features=self.hidden_size * 2 * 3 * 2)
            else:
                self.concat_output_BN = nn.BatchNorm1d(num_features=self.hidden_size * 2 * 2)
    ...
    def forward(self, X_q_inputs, X_r_inputs, q_init_state, r_init_state):
        ...
        # output = self.out(self.dropout(F.tanh(concat_outputs)))
        return output
...
def train_epoch(model, data_train):
    ...
    for batch in range(tr_batch_num):
        ...
        if (batch + 1) % display_batch == 0:
            valid_acc, valid_cost = test_epoch(model, data_test, valid=True)
    ...
    test_acc, test_cost, test_cr, disagree_f1, agree_f1 = test_epoch(model, data_test, valid=False)
    average_f1 = (disagree_f1 + agree_f1) / 2

    print('**Test {}, acc={:.6f}, cost={:.6f}, average_f1={:.6f}\n'.format(data_test.y.shape[0], test_acc, test_cost, average_f1))
    fw.write('**Test {}, acc={:.6f}, cost={:.6f}, average_f1={:.6f}\n'.format(data_test.y.shape[0], test_acc, test_cost, average_f1))
...
def test_epoch(model, dataset, valid):
    model.eval()
    ...
    for i in range(batch_num):
        ...
        _acc = sum(y_pred.data.cpu() == y_batch.data.cpu()) / float(y_batch.size(0)) if USE_GPU else sum(
            y_pred.data == y_batch.data) / float(y_batch.size(0))
    ...
    if not valid:
        ...
        return mean_acc, mean_cost, cr, f1[0], f1[1]
    else:
...
if __name__ == '__main__':
    ...
    # for type in ['None', 'hybrid']: # gpu:0
    # for type in ['self_attention', 'cross_attention']: # gpu:1
    # for type in ['both_sum', 'both_concat']: # gpu:2
    # for type in ['None', 'self_attention', 'cross_attention', 'both_sum', 'both_concat', 'hybrid']:  # gpu:3
    # for type in ['match']: # gpu:0
    for type in ['hybrid']: # gpu:0
        ...
        for i, (train_index, test_index) in enumerate(kf.split(X)):
            ...
            # with open('./result_log/debatepedia_%s_%s.txt' % (type, i), 'w') as fw:
                ...
                for epoch in range(1, max_max_epoch + 1):
                    ...
                    # fw.write('EPOCH {}, lr={}\n'.format(epoch, lr))
                    #
                    # current_result = train_epoch(model, data_train)
                    #
                    ...
                    #     lr_decay_counter += 1
                    #
                    # print('BEST_EPOCH={}, BEST_RESULT={:.6f}, lr_decay_counter={}, stop_counter={}'.format(best_epoch, best_result, lr_decay_counter, stop_counter))
                    # fw.write('BEST_EPOCH={}, BEST_RESULT={:.6f}\n'.format(best_epoch, best_result))
                    #
                    # if lr_decay_counter == LR_DECAY:
                    #     for param_group in optimizer.param_groups:
                    #         param_group['lr'] *= 0.5
                    #     stop_counter += 1
                    #     lr_decay_counter = 0
