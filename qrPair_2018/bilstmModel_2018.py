...
class TextBiLSTM(object):
    def __init__(self, wordEmbedding, textLength, vocabulary_size,
                 ...
                 class_num, do_BN, attention_mechanism, max_grad_norm):
...
if __name__ == '__main__':
    ...
    model = TextBiLSTM(wordEmbedding=None, textLength=max_len, vocabulary_size=len(word2id)+1,
                       embedding_size=300, hidden_size=128, layer_num=3,
                       class_num=3, do_BN=True, attention_mechanism=attention_mechanism_config, max_grad_norm=0.5)
    ...
    for epoch in range(1, max_max_epoch+1):
        ...
        for batch in range(tr_batch_num):
            ...
            if (batch + 1) % display_batch == 0:
                valid_acc, valid_cost, _ = test_epoch(sess, model, data_test)
        ...
        test_acc, test_cost, test_cr = test_epoch(sess, model, data_test)
